var map = L.map('map').setView([11.013277129, 76.932855015], 8); // global leaflet map element
var selectedService; // current selected service (cosm or tnau_cbe)
var checkedStation; // current selected station (aqe or tnau_cbe station)
var selectedMarker; // current selected leaflet marker
var results; // contains all data of the current selected aqe
var tnau_cbeStationen; // contains all tnau_cbe stations
...
var tnau_cbe_selected = L.icon({
    iconUrl: 'img/aws_selected.png',
    iconSize: [41, 41],
    iconAnchor: [25, 40],
    popupAnchor: [-3, -40],
});
...
//If it fails load tnau_cbe data and give feedback to the user.
function loadCOSM()
{
...
//Adds the offical stationen.js script of tnau_cbe to our website.
function loadtnau_cbe() {
	jQuery.ajax({
        url: 'http://localhost/qualitySCHU/stationen.js',
        type: 'get',
        dataType: 'script',
        success:addtnau_cbe
     });
...
//Success function of loadtnau_cbe function. First create a marker and add it to the default clustergroup.
function addtnau_cbe() {
	tnau_cbeStationen = stationen; //sets all available stations for later use
	for (station in stationen) {
		marker = new L.Marker(new L.LatLng(stationen[station].latitude,stationen[station].longitude),{icon:tnau_cbe, title:station});
		clusters.addLayer(marker);
	}
...
clusters.on('click', function(e) {
	...
	//Check what marker type is selected. If selectedMarker title is not a number a tnau_cbe stations is selected else if it is a number a AQE is selected.
	if (selectedMarker!= null && isNaN(selectedMarker.options.title)) {
		selectedMarker.setIcon(tnau_cbe);
	...
	if (isNaN(checkedStation)) {
		...
		$('.carousel').carousel(1); //switch over to the tnau_cbe sensorbar
		selectedMarker.setIcon(tnau_cbe_selected);
		//var tsp = new Array();		
		selectedService = "tnau_cbe";
		jQuery.ajax({
			url: 'http://localhost/istsos/wa/istsos/services/tnau_cbe/operations/getobservation/offerings/temporary/procedures/'+checkedStation+'/observedproperties/urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:temperature,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:humidity,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:ws,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:wd,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:ap/eventtime/2013-10-23T08:59:00+0530/2013-10-23T10:00:00+0530?_dc=1363547035889',
	        type: 'get',
	        dataType: "json",
	        success:function(data3) {
			data1 = data3;
	        	console.log(data1);
			for (var i = 0; i < data1.data[0].result.DataArray.values.length; i++) {
				td = data1.data[0].result.DataArray.values[i];
		 	$('#tnau_cbeTempForm').text(function(){
	        		return (isNaN(data3.data[0].result.DataArray.values[0][5])) ? "-" : new Number (data3.data[0].result.DataArray.values[0][5]).toFixed(1);
	        	});
	        	$('#tnau_cbeHumForm').text(function() {
	        		return (isNaN(data3.data[0].result.DataArray.values[0][3])) ? "-" : new Number (data3.data[0].result.DataArray.values[0][3]).toFixed(1);
	        	});
	        	$('#tnau_cbeWSpeedForm').text(function() {
	        		return (isNaN(data3.data[0].result.DataArray.values[0][9])) ? "-" : new Number(data3.data[0].result.DataArray.values[0][9]).toFixed(1);
	        	});
	        	$('#tnau_cbeWDireForm').text(function() {
	        		return (isNaN(data3.data[0].result.DataArray.values[0][7])) ? "-" : new Number(data3.data[0].result.DataArray.values[0][7]).toFixed(1);
	        	});
	        	$('#tnau_cbeAtmPreForm').text(function() {
...
function updateTable(start,end) {
	...
	if (selectedService == "tnau_cbe") {
		url = 'http://localhost/istsos/wa/istsos/services/'+selectedService+'/operations/getobservation/offerings/temporary/procedures/'+checkedStation+'/observedproperties/urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:temperature,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:humidity,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:ws,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:wd,urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:ap/eventtime/'+eventtime_start+'/'+eventtime_end+'?_dc=1363547035889'
	} 
		...
		jQuery.ajax({
	        ...
	        success:function(data3) {
	        	...
	        	for (var i = 0; i < data2.data[0].result.DataArray.values.length; i++) {
	        		...
	        		tempdate = td[0].split("+");
		   		tempdate = tempdate[0]+"+00:00";
					date = new Date(tempdate).toString('dd.MM.yyyy HH:mm:ss');
					
					//Check the data quality and depending on that set the color
		        	//for (var j = 2; j < td.length; j = j - 3) {
			        	if(td[i]=="100") {
				        	newtablerow = newtablerow + '<td class="tdwarning">'+td[i-1]+'</td>';
			        	} else if(td[i]=="101") {
				        	newtablerow = newtablerow + '<td class="tderror">'+td[i-1]+'</td>';
			        	} else if(td[i]=="102") {
				        	newtablerow = newtablerow + '<td class="tdsuccess">'+td[i-1]+'</td>';
			        	} else {
...
function addToDiagram(elems,start,end) {
	...
	for (var i = 0; i < elems.length; i++) {
		if (query == "") {
			query = "urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:"+ elems[i];	
		} else {
			query = query + "urn:ogc:def:parameter:x-istsos:1.0:tnau:aws:" + elems[i];
		}
	}
	...
	eventtime_end = eventtime_end + getTz(end.getTimezoneOffset());
	jQuery.ajax({
        url: 'http://localhost/istsos/wa/istsos/services/'+selectedService+'/operations/getobservation/offerings/temporary/procedures/'+checkedStation+'/observedproperties/'+query+'/eventtime/'+eventtime_start+'/'+eventtime_end+'?_dc=1363547035889',
        type: 'get',
        dataType: "json",
        success:function(data3) {
